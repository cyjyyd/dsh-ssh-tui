...
export const en: Record<string, string> = {
  ...
  'diag.rowTerminal': 'terminal: {label} ({family}) · mouse {mouse} · bracketed paste {paste} · alt screen {alt} · OSC52 {osc52} · OSC8 {osc8} · title {title}{ignored}',
  ...
  'usage.a2NoCred': 'AIClient2API quota needs a panel credential: set {env} (the panel login password) or {token} (a token from a browser session)',
  'usage.a2Login': '{label} panel login failed (HTTP {status}) — AICLIENT2API_PASSWORD is the panel password, not the API key',
  'usage.a2LoginDetail': '{label} panel login rejected (HTTP {status}): {detail}',
  'usage.a2NoToken': '{label} panel login succeeded but returned no token',
  ...
  'copy.osc52Hint': 'This terminal ({terminal}) does not accept OSC 52 clipboard writes, so that copy may not have landed; select the text with Shift+drag and copy it with the terminal\'s own shortcut, or use a terminal that supports OSC 52 (Windows Terminal, Konsole 24.12+). Shown once per session.',
