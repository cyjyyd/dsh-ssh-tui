...
export const en: Record<string, string> = {
  ...
  'footer.agentPlaneMissing': 'agent-plane rows missing (/doctor)',
  ...
  'diag.host.live': 'alive (display channel answers)',
  ...
  'diag.rowTerminal': 'terminal: {label} ({family}) · mouse {mouse} · bracketed paste {paste} · alt screen {alt} · OSC52 {osc52} · OSC8 {osc8} · title {title}{ascii}{ignored}',
  'diag.asciiOn': ' · glyphs ASCII fallback (console is not UTF-8; chcp 65001 or Windows Terminal restores them)',
  'diag.terminalIgnored': ' · rejected DSH_TUI_TERM_CAPS: {tokens}',
  ...
  'preset.trustManaged': 'host-managed',
  'preset.directoryManaged': '(this host exposes no directory)',
  'mode.noRecompose': 'this host cannot switch presets in place ({id}): restart the session instead',
  ...
  'onboard.modelsPick': 'Choose the models to configure ({count} available, digits/letters toggle):',
  'onboard.defaultModelPick': 'Choose the model this session should run:',
  'onboard.modelsPickHint': '  Enter = open the model picker (template models pre-checked)',
  'onboard.modelsCheckHint': '  ↑/↓ move · space or digit/letter toggles · Enter confirms · Esc goes back (past 9+26 use ↑/↓)',
  ...
  'onboard.confirmDefaultModel': '  session model: {model}',
  ...
  'onboard.providerSplit': '{id}: models routed by protocol ({summary})',
  'onboard.protocolAuto': 'protocol chosen per model',
  ...
  'model.wrongRoute': 'Model {model} is not served by provider {provider}: this row speaks {endpoint}. {hint}',
  'model.wrongRouteTarget': '{provider} serves that model — pick it with /provider and it is filed on the route that speaks its protocol.',
  'model.wrongRouteSetup': 'The gateway has no row for it yet — create one with /setup first.',
  ...
  'mode.missingServiceHint': 'Fix: run /mode fix to add the row to {patch}, then restart the TUI; a checkout can also run node scripts/profile-rows.mjs {profile} (bash scripts/ensure-profile-rows.sh {profile} is the same).',
  ...
  'mode.formsHint': 'If the persona / ask_user_question / present tools are missing, run /mode fix to add those rows to {patch}, then restart the TUI; a checkout can also run node scripts/profile-rows.mjs {profile} (bash scripts/ensure-profile-rows.sh {profile} is the same).',
  ...
  'cmd.notify': 'notify when a question waits and nobody is attached: /notify shows it; /notify off clears it; /notify mail <address>; /notify smtp <host[:port]> <from> <to> [user password]; /notify local [port] <from> <to>',
  ...
  'attach.questionWaited': 'this question has been waiting {waited}',
  'notify.mailSubject': 'dsh is waiting on your answer',
  'notify.usage': 'usage: /notify shows the current setting; /notify off clears it; /notify mail <address> uses the local mailer; /notify smtp <host[:port]> <from> <to> [user password] sends through SMTP (port 587 when omitted); /notify local [port] <from> <to> uses the local mailer (port 25 when omitted).',
  'notify.statusOff': 'question notify: off. A waiting question still leaves a .waiting marker in tui-socks.',
  'notify.statusOn': 'question notify: {command}',
  'notify.cleared': 'question notify turned off.',
  'notify.saved': 'question notify set to {target}. A question that starts waiting with nobody attached sends once.',
  'notify.body': 'dsh session {session} has {count} question(s) waiting ({waited} so far)',
  'notify.resume': 'reconnect: {command}',
  ...
  'attach.attached': 'Session {session} is already attached in another window (pid {pid}). Start the resume picker (`dsh --profile tui`) and pick that session to confirm taking it over.',
  ...
  'changes.header': 'changes this turn · {count} files  +{added} -{deleted}',
  'changes.binary': 'binary',
  'changes.oversized': 'oversized',
  'changes.more': '{count} more not listed',
  'changes.coarse': 'change too large; shown as a whole-file replacement',
  'changes.binaryFile': 'binary file; nothing to display',
  'changes.oversizedFile': 'file too large; nothing to display',
  'changes.unavailable': 'these changes are no longer available (not kept across a restart)',
  'changes.inspectTitle': 'changes this turn · {file}',
}
